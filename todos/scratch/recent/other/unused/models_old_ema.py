...
class ModelObj:
    ...
    def __init__(self, trainer):
        ...
        if conf.use_ema:
            ...
            # mark: 
            model_ema_steps = self.conf.update_ema_model_every_n_steps
            batch_size = self.conf.bsz
            model_ema_decay = self.conf.ema_decay
            epochs = self.conf.total_approx_epochs
            device = self.trainer.device 

            # PyTorch / Jiachen
            #https://github.com/pytorch/vision/blob/0d75d9e5516f446c9c0ef93bd4ed9fea13992d06/references/classification/train.py#L328
            ...
            adjust = batch_size * model_ema_steps / epochs
            alpha = 1.0 - model_ema_decay
            alpha = min(1.0, alpha * adjust)
            self.ema_model = ExponentialMovingAverage(self.model, device=device, decay=1.0 - alpha)
    ...
    def custom_update_ema_model(self,):
        
        step = self.get_step()

        if not self.conf.use_ema:
            return

        if step % self.conf.update_ema_model_every_n_steps == 0:

            self.ema_model.update_parameters(self.model)
            if step < self.conf.num_warmup_steps:
                # Reset ema buffer to keep copying weights during warmup period
                self.ema_model.n_averaged.fill_(0)
                print("reset ema buffer b/c in LR warmup")
            print("*"*30)            
