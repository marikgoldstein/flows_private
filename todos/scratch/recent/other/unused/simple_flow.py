...
def one_training_step(q0, q1, model, optimizer):
    optimizer.zero_grad()
...
def training_loop(step):

    for rep in range(50):

        one_training_step(q0, q1, model, optimizer)
...
if __name__=='__main__':
    ...
    # init stuff
    gmm = GMM(GMM_Config(layout='flow', N0=1, N1=2))
    ...
    plt.xlim([-20, 20])
    plt.ylim([-20, 20])
    ani = animation.FuncAnimation(fig, training_loop, repeat=False, frames=100)
    plt.show()
