...
class NCSN(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.logit_transform = config.logit_transform
        self.rescaled = config.rescaled
        self.norm = get_normalization(config, conditional=True)
        self.ngf = ngf = config.model_ngf
        self.num_classes = num_classes = config.model_num_classes
        self.act = act = get_act(config)
        self.config = config
        
        self.data_channels = config.C
        self.image_size = config.H

        self.begin_conv = nn.Conv2d(self.data_channels, ngf, 3, stride=1, padding=1)
        self.spec_norm = spec_norm = config.model_spec_norm
...
class NCSNdeeper(nn.Module):
    def __init__(self, config):
        ...
        self.end_conv = nn.Conv2d(ngf, self.data_channels, 3, stride=1, padding=1)
        spec_norm = config.model_spec_norm
